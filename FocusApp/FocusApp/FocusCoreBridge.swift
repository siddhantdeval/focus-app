...
// is now provided via the FocusApp-Bridging-Header.h
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_focus_core_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_focus_core_rustbuffer_free(self, $0) }
    }
}
...
@_documentation(visibility: private)
#endif
public struct FfiConverterTypeTimerState: FfiConverterRustBuffer {
    typealias SwiftType = TimerState

    public static func read(from buf: inout (data: Data, offset: Data.Index)) throws -> TimerState {
        let variant: Int32 = try readInt(&buf)
        switch variant {
    ...
    public static func write(_ value: TimerState, into buf: inout [UInt8]) {
        switch value {
...
@_documentation(visibility: private)
#endif
public func FfiConverterTypeTimerState_lift(_ buf: RustBuffer) throws -> TimerState {
    return try FfiConverterTypeTimerState.lift(buf)
}

#if swift(>=5.8)
@_documentation(visibility: private)
#endif
public func FfiConverterTypeTimerState_lower(_ value: TimerState) -> RustBuffer {
    return FfiConverterTypeTimerState.lower(value)
}



extension TimerState: Equatable, Hashable {}
...
fileprivate struct UniffiCallbackInterfaceTimerObserver {
    ...
    // Create the VTable using a series of closures.
    // Swift automatically converts these into C callback functions.
    static var vtable: UniffiVTableCallbackInterfaceTimerObserver = UniffiVTableCallbackInterfaceTimerObserver(
        onTick: { (
            uniffiHandle: UInt64,
            ...
            let makeCall = {
                () throws -> () in
                guard let uniffiObj = try? FfiConverterCallbackInterfaceTimerObserver.handleMap.get(handle: uniffiHandle) else {
                    throw UniffiInternalError.unexpectedStaleHandle
                }
                return uniffiObj.onTick(
                     remainingSeconds: try FfiConverterUInt32.lift(remainingSeconds)
                )
            }
        ...
        onStateChanged: { (
            ...
            let makeCall = {
                ...
                return uniffiObj.onStateChanged(
                     state: try FfiConverterTypeTimerState.lift(state)
                )
            }
        ...
        uniffiFree: { (uniffiHandle: UInt64) -> () in
            let result = try? FfiConverterCallbackInterfaceTimerObserver.handleMap.remove(handle: uniffiHandle)
            if result == nil {
                print("Uniffi callback interface TimerObserver: handle missing in uniffiFree")
            }
        }
    )
}

private func uniffiCallbackInitTimerObserver() {
    uniffi_focus_core_fn_init_callback_vtable_timerobserver(&UniffiCallbackInterfaceTimerObserver.vtable)
}
...
@_documentation(visibility: private)
#endif
fileprivate struct FfiConverterCallbackInterfaceTimerObserver {
    fileprivate static var handleMap = UniffiHandleMap<TimerObserver>()
}

#if swift(>=5.8)
@_documentation(visibility: private)
#endif
extension FfiConverterCallbackInterfaceTimerObserver : FfiConverter {
    typealias SwiftType = TimerObserver
    typealias FfiType = UInt64
...
public func startTimer(durationSeconds: UInt32, observer: TimerObserver) {try! rustCall() {
    uniffi_focus_core_fn_func_start_timer(
        ...
        FfiConverterCallbackInterfaceTimerObserver.lower(observer),$0
    )
}
}
public func stopTimer() {try! rustCall() {
    uniffi_focus_core_fn_func_stop_timer($0
    )
}
}
...
private var initializationResult: InitializationResult = {
    ...
    let scaffolding_contract_version = ffi_focus_core_uniffi_contract_version()
    ...
    if (uniffi_focus_core_checksum_method_timerobserver_on_state_changed() != 28551) {
        return InitializationResult.apiChecksumMismatch
    }
